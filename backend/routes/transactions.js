...
// GET /api/transactions - Retrieve transactions with pagination, filter (time range, hash) and sorting
router.get('/', (req, res) => {
  const { page = 1, pageSize = 10, txHash = '', startDate, endDate } = req.query;
  const filteredTx = transactions
    .filter(tx => tx.hash.toLowerCase().includes(txHash.toLowerCase()))
    .filter(tx => {
